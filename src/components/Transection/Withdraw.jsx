...
const Withdraw = () => {
    ...
    const onSubmit = (data) => {
      ...
      if(userInfo.balance>=data.amount){
        if(data.amount>=10){
          axios.put(`/withdraw/${userInfo?.email}`,{amount:data?.amount})
          .then(()=>{
          ...
          Swal.fire({
            title: 'Error!',
            text: 'oopss insuficient balance',
            icon: 'error',
    ...
    return (
        ...
        <h2 className="text-2xl my-3 font-bold text-center">Please fill up the form to withdraw
        </h2>
        <form
